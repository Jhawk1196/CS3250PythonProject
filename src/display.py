...
class Display:  # pragma: no cover
    ...
    def display(
        ...
        cycle_menu.add_command(label="5 seconds", command=lambda: [
            self.time_var.set(5000), root.update()])
        cycle_menu.add_command(label="10 seconds", command=lambda: [
            self.time_var.set(10000), root.update()])
        cycle_menu.add_command(label="15 seconds", command=lambda: [
            self.time_var.set(15000), root.update()])
        cycle_menu.add_command(label="30 seconds", command=lambda: [
            self.time_var.set(30000), root.update()])
        cycle_menu.add_command(label="60 seconds", command=lambda: [
            self.time_var.set(60000), root.update()])
        ...
        font_menu.add_command(label="Times New Roman", command=lambda: [
            fontSelect.font_style(custom_font, 'Times'),
            self.family.set('Times'), root.update()])
        font_menu.add_command(label="Arial", command=lambda: [
            fontSelect.font_style(custom_font, 'Arial'),
            self.family.set('Arial'), root.update()])
        font_menu.add_command(label="Helvetica", command=lambda: [
            fontSelect.font_style(custom_font, 'Helvetica'),
            self.family.set('Helvetica'), root.update()])
        font_menu.add_command(label="Verdana", command=lambda: [
            fontSelect.font_style(custom_font, 'Verdana'),
            self.family.set('Verdana'), root.update()])
        font_menu.add_command(label="Wingdings", command=lambda: [
            fontSelect.font_style(custom_font, 'Wingdings'),
            self.family.set('Wingdings'), root.update()])
        ...
        size_menu.add_command(label="10 pt", command=lambda: [
            fontSelect.font_size(custom_font, 10), self.size.set(10),
            root.update()])
        size_menu.add_command(label="11 pt", command=lambda: [
            fontSelect.font_size(custom_font, 11), self.size.set(11),
            root.update()])
        size_menu.add_command(label="12 pt", command=lambda: [
            fontSelect.font_size(custom_font, 12), self.size.set(12),
            root.update()])
        size_menu.add_command(label="13 pt", command=lambda: [
            fontSelect.font_size(custom_font, 13), self.size.set(13),
            root.update()])
        size_menu.add_command(label="14 pt", command=lambda: [
            fontSelect.font_size(custom_font, 14), self.size.set(14),
            root.update()])
        size_menu.add_command(label="15 pt", command=lambda: [
            fontSelect.font_size(custom_font, 15), self.size.set(15),
            root.update()])
        size_menu.add_command(label="16 pt", command=lambda: [
            fontSelect.font_size(custom_font, 16), self.size.set(16),
            root.update()])
        ...
        font_color.add_command(label="Black", command=lambda: [
            fontSelect.font_color(self.label, 'black'),
            self.color.set('black'), root.update()])
        font_color.add_command(label="White", command=lambda: [
            fontSelect.font_color(self.label, 'white'),
            self.color.set('white'), root.update()])
        font_color.add_command(label="Red", command=lambda: [
            fontSelect.font_color(self.label, 'red'),
            self.color.set('red'), root.update()])
        font_color.add_command(label="Blue", command=lambda: [
            fontSelect.font_color(self.label, 'blue'),
            self.color.set('blue'), root.update()])
        font_color.add_command(label="Green", command=lambda: [
            fontSelect.font_color(self.label, 'green'),
            self.color.set('green'), root.update()])
    ...
    def save_values(
        ...
        save_dict = {'time': self.time_var.get(), 'font_size': self.size.get(),
                     'font_color': self.color.get(),
                     'background_color': self.background.get(), 'font_family':
                         self.family.get(),
    ...
    def display_message(
            self, args):  # pragma: no cover
        """
        Governs what messages are displayed in GUI and sets the URL to each
        feed to open in browser
        """
